...
/**
 * 
 * QQ第2代加密算法java版 Created on 2005-10-26
 * 
 * @author admundliu
 * @version 1.0
 * 
 * Modified By frankywang 
 * 增加decrypt(byte[] in, byte[] key, int length)，原来的decrypt(byte[] in, byte[] key)不建议使用
 **/
...
public class Cryptor {
	...
	public static byte[] _4bytesEncryptAFrame(int data, byte[] key) {
		short[] v = new short[2];
		v[0] = (short) (data & 0xffff);
		v[1] = (short) (data >>> 16);
		short[] k = new short[4];
		k[0] = (short) ((key[1] << 8) | key[0]);
		k[1] = (short) ((key[3] << 8) | key[2]);
		k[2] = (short) ((key[5] << 8) | key[4]);
		k[3] = (short) ((key[7] << 8) | key[6]);
	 ...
	 * 我们以prePlain表示前一个明文块，plain表示当前明文块，crypt表示当前明文块加密得到的密文块，preCrypt表示前一个密文块 f表示加密算法，d表示解密算法 那么从plain得到crypt的过程是: crypt
	 * = f(plain &circ; preCrypt) &circ; prePlain 所以，从crypt得到plain的过程自然是 plain = d(crypt &circ; prePlain) &circ;
	 * preCrypt 此外，算法有它的填充机制，其会在明文前和明文后分别填充一定的字节数，以保证明文长度是8字节的倍数 填充的字节数与原始明文长度有关，填充的方法是:
	 * 
	 * <pre>
	 * &lt;code&gt;
	 * 
	 ...
	 */
	/**
	 * 把字节数组从offset开始的len个字节转换成一个unsigned int， 因为java里面没有unsigned，所以unsigned int使用long表示的，
	 * 如果len大于8，则认为len等于8。如果len小于8，则高位填0 <br>
	...
	public byte[] decrypt(byte[] in, byte[] key) {
		return decrypt(in, 0, in.length, key);
	}
	...
	// public int findEncryptSize( int nLen )
	// {
	...
	// nPadSaltBodyZeroLen =
	// nLen/*Body长度*/+1+SALT_LEN+ZERO_LEN/*PadLen(1byte)+Salt(2byte)+Zero(7byte)*/;
	// nPadlen = nPadSaltBodyZeroLen%8;
	// if(nPadlen > 0 ) /*len=nSaltBodyZeroLen%8*/
	// {
	...
	public byte[] decrypt(byte[] in, byte[] key, int length) {
		byte[] data = decrypt(in, 0, in.length, key);
	...
	private byte[] getRandomByte(int length){
		byte[] data = new byte[length]; //得到随机长度的数据
